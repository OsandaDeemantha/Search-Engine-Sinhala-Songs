...
def search(phrase):
    ...
    for i in range(2, 9):
        if phrase in all_lists[i]:
            ...
            flags[i] = 5
    # If there are more than 8 words,boost lyrics
    if len(tokens) > 8:
...
syn_lyrics = ['ගත්කරු','රචකයා','ලියන්නා','ලියන','රචිත','ලියපු','ලියව්‌ව','රචනා','රචක','ලියන්', 'ලියූ']
